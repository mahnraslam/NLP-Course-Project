const S = {
  card: (score) => ({
    background: '#111e30', borderLeft: `3px solid ${score > 0.7 ? '#2563eb' : score > 0.4 ? '#7c3aed' : '#374151'}`,
...
export default function CitationCard({ citation, onJumpToPage }) {
  const { filename, page_num, chunk_text, relevance_score } = citation
  ...
  return (
    <div style={S.card(score)} onClick={handleJump}>
      ...
      <div style={S.quote}>"{chunk_text?.slice(0, 180)}{chunk_text?.length > 180 ? '…' : ''}"</div>
      <div style={S.hint}>Relevance: {(score * 100).toFixed(0)}% · Click to view in blueprint →</div>
    </div>
  )
}
